...
let val_fun p ~ret_t f =
  ...
  let ffi = { ffi_args = p'; ffi_applied = []; ffi_eval = f; ffi_meta = false; ffi_external = None } in
    mk ~t (FFI ffi)
...
let val_cst_fun p c =
  ...
  in
    ...
    match c.value with
      ...
      | _ ->
        let ffi = { ffi_args = p'; ffi_applied = []; ffi_eval = (fun _ _ -> c); ffi_meta = false; ffi_external = None } in
        mk ~t (FFI ffi)
...
let add_builtin ~category ~descr ?(flags=[]) ?(meta=false) ?extern name proto return_t f =
  let t = builtin_type proto return_t in
  let f env t = f (List.map (fun (s,(l,v)) -> assert (l=[]) ; s,v) env) t in
  let ffi = { ffi_args = List.map (fun (lbl,_,opt,_) -> lbl,lbl,opt) proto; ffi_applied = []; ffi_eval = f; ffi_meta = meta; ffi_external = extern } in
  let value =
    { t = t ;
      value = FFI ffi }
  in
...
let iter_sources f v =
  let rec iter_term env v = match v.Term.term with
    ...
    | Term.Is_field _ -> ()
    | Term.Open (r,t) ->
      (* TODO: is that correct wrt to environment? *)
